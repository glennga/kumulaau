...
class Distance(ABC):
    ...
    def _choose_ell_0(self, kappa: int, omega: int) -> ndarray:
        ...
        return array([min(omega, max(kappa, choice(self.ell_0_pool)))])
    ...
    def fill_matrices(self, theta_proposed: BaseParameters, epsilon: float) -> float:
        """ Compute the expected distance for all observations to a population generated by our proposed parameter set.
        ...
        # Generate all of our phylogeny trees and save the generated data we are to compare to.
        sample_all = array([Population(theta_proposed).evolve(
            self._choose_ell_0(theta_proposed.kappa, theta_proposed.omega)) for _ in range(self.h.shape[0])])
...
if __name__ == '__main__':
    ...
    main_theta = {'n': 100, 'f': 1.0, 'c': 0.01, 'u': 1.2, 'd': 0.001, 'kappa': 3, 'omega': 30}
    ...
    start_t = timer()
    expected_delta = main_accumulator.fill_matrices(BaseParameters(**main_theta), 0.1)
    end_t = timer()
