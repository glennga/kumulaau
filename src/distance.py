...
class Summary(ABC):
    ...
    def _prepare(self, observed_frequency_dirty: List) -> None:
        ...
        self.generated_frequency, self.observed_frequency, self.distances = \
            [zeros(self.omega), zeros(self.omega), zeros(1)]  # TODO: Convert self.distances into not a list...
...
class Cosine(Summary):
    # If our distance function returns a 0, then we have found a orthogonal vector. This is the "worst case" scenario.
    COMPLETE_DIFFERENCE = 0

    # If our distance function returns a 1, then we have found an exact match.
    COMPLETE_MATCH = 1
