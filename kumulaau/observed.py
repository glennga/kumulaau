...
class Observed(object):
    ...
    @classmethod
    def single_insert(cls, cursor: Cursor, entry: SimpleNamespace) -> None:
    ...
    @classmethod
    def extract_tuples(cls, filename: str, uid_loci: List) -> List:
        ...
        if not bool(cursor.execute(f"""
            SELECT NAME
            FROM sqlite_master
            WHERE type='table' AND NAME='{cls.TABLE_NAME}'
        """).fetchone()):
        ...
        tuples = cls._extract_tuples(cursor, uid_loci)
    ...
    def tuples_to_sparse_matrix(tuples: List, bounds: List) -> ndarray:
        ...
        observation_dictionary = Observed.tuples_to_dictionaries(tuples)
    ...
    def tuples_to_pool(tuples: List) -> ndarray:
        ...
        """
        return Observed.tuples_to_numpy(tuples)[:, :, 0].flatten()
